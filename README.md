...
• The Jenkins job starts automatically.	⚠️ Common Issues
...
A webhook is an HTTP callback. One system sends an HTTP request to another system when an event happens. No polling is required, so the flow is event-driven.
